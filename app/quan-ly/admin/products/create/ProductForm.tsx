...
import { Trash2, Plus, Info, DollarSign, Settings, Users, ListChecks, MapPin, CheckSquare } from "lucide-react";
import { v4 as uuidv4 } from "uuid";

// Định nghĩa kiểu dữ liệu cho Feature trong Form
...
// Giá trị mặc định cho Form Data
...
export function ProductForm({ initialData, onSubmit, isEditMode, onDelete, isLoading = false }: ProductFormProps) {
  ...
  const validateForm = () => {
    ...
    else if (formData.accountManager?.email && !/\S+@\S+\.\S+/.test(formData.accountManager.email))
      newErrors.accountManagerEmail = "Email không hợp lệ.";
  ...
  const addFeature = () => {
    setFormData((prev) => ({
      ...prev,
      features: [...(prev.features || []), { id: uuidv4(), title: "", description: "", icon: "" }],
    }));
  };
    ...
    <form onSubmit={handleFormSubmit} className="space-y-10 p-4 md:p-6 bg-gray-50 dark:bg-gray-900 rounded-lg shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ví dụ: Gói Carbon Tín chỉ Rừng XYZ"
              disabled={isLoading}
            />
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <Input
                ...
                onChange={handleInputChange}
                placeholder="Địa điểm cụ thể hoặc vùng"
                disabled={isLoading}
                className="flex-1"
              />
            </div>
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <Input
                ...
                onChange={handleInputChange}
                placeholder="Ví dụ: Verra VCS, Gold Standard"
                disabled={isLoading}
                className="flex-1"
              />
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
          <div>
            <Label htmlFor="accountManagerName">
              Tên người quản lý <span className="text-destructive">*</span>
            </Label>
            <Input
          ...
          <div>
            <Label htmlFor="accountManagerEmail">
          ...
          <div>
            <Label htmlFor="accountManagerPhone">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        <div className="space-y-4">
          {(formData.features || []).map((feature, index) => (
            <div
              key={feature.id}
              className="border border-dashed dark:border-gray-700 p-4 rounded-md space-y-3 bg-gray-50 dark:bg-gray-700/30 relative"
            >
              ...
              <div>
                <Label htmlFor={`featureTitle${feature.id}`}>
                  Tên tính năng <span className="text-destructive">*</span>
                </Label>
                <Input
                  id={`featureTitle${feature.id}`}
                  placeholder="Ví dụ: Báo cáo tùy chỉnh"
                  value={feature.title}
                  onChange={(e) => updateFeature(index, "title", e.target.value)}
                  disabled={isLoading}
                />
                {errors[`featureTitle${index}`] && (
                  <p className="text-sm text-destructive mt-1">{errors[`featureTitle${index}`]}</p>
                )}
              </div>
              <div>
                <Label htmlFor={`featureDescription${feature.id}`}>
                  Mô tả tính năng <span className="text-destructive">*</span>
                </Label>
                <Textarea
                  id={`featureDescription${feature.id}`}
                  placeholder="Mô tả chi tiết lợi ích của tính năng"
                ...
                {errors[`featureDescription${index}`] && (
                  <p className="text-sm text-destructive mt-1">{errors[`featureDescription${index}`]}</p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="Từ thư viện Lucide React hoặc tên class FontAwesome"
                  value={feature.icon}
