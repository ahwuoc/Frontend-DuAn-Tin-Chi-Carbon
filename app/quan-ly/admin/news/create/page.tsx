...
export default function NewsFormPage() {
        ...
        if (newsId) {
            ...
            const fetchNews = async () => {
                try {
                    ...
                    toast({ title: "Lỗi", description: "Không thể tải dữ liệu tin tức.", variant: "destructive" });
                } finally {
                    setIsFetching(false);
                }
            };
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        let payloadUserId = formData.userId;
        if (!newsId) {
            const storedUserId = localStorage.getItem("user_id");
            if (storedUserId) {
                try {
                    payloadUserId = JSON.parse(storedUserId) || formData.userId || "";
                } catch {
                    payloadUserId = formData.userId || "";
                }
            }
        }

        if (!payloadUserId && !newsId) {
            toast({ title: "Thiếu User ID", description: "Không thể tạo tin tức nếu không có User ID.", variant: "destructive" });
            setLoading(false);
            return;
        }
        ...
        try {
            if (newsId) {
                await apiNews.update(newsId, payload as INews);
            ...
            toast({
                title: "Lỗi",
                description: err.message || "Không thể xử lý tin tức!",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <CardFooter className="flex justify-end gap-3 mt-8 p-0">
                            <Button type="button" variant="outline" onClick={() => router.push("/quan-ly/admin/news")} disabled={loading || isFetching || isUploadingImage} size="lg">
