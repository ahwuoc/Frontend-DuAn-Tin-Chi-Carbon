...
export default function NewsFormPage() {
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'prose dark:prose-invert max-w-none min-h-[200px] outline-none p-3', // Thêm padding và class cho styling nội dung
            },
        },
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const isContentEmpty = !formData.content || formData.content.trim() === "" || formData.content.trim() === "<p></p>";
        if (!formData.title || isContentEmpty || !formData.category || !newsId) {
            toast({ title: "Thiếu thông tin", description: "Vui lòng điền Tiêu đề, Nội dung, Danh mục và ID tin tức.", variant: "destructive" });
            return;
        }
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <CardFooter className="flex justify-end gap-3 mt-8 p-0">
                            <Button type="button" variant="outline" onClick={() => router.push("/quan-ly/admin/news")} disabled={loading || isFetching || isUploadingImage || !editor} size="lg">
                                <XCircle className="mr-2 h-5 w-5" /> Hủy
                            </Button>
                            <Button type="submit" disabled={loading || isFetching || isUploadingImage || !editor} size="lg" className="bg-green-600 hover:bg-green-700 text-white">
