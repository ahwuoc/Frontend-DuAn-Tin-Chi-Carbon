  ...
  return (
    <article className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-4xl mx-auto">
        {" "}
        {/* Điều chỉnh max-w- tùy theo thiết kế mong muốn (ví dụ: max-w-screen-md, max-w-lg, max-w-xl, max-w-4xl...) */}
        <header className="mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-gray-100 leading-tight mb-4">
            {" "}
            {/* Kích thước tiêu đề, độ đậm, khoảng cách dòng */}
            {newsItem.title}
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-gray-500 dark:text-gray-400 text-sm">
            {" "}
            {/* Sử dụng flex-wrap cho responsive, điều chỉnh gap */}
            {newsItem.createdAt && (
        ...
        {newsItem.image && (
          <div className="relative w-full h-60 md:h-80 lg:h-96 rounded-lg overflow-hidden mb-10 shadow-lg">
            {" "}
            {/* Tăng khoảng cách dưới ảnh, thêm shadow */}
            <Image
              src={newsItem.image}
        ...
        {newsItem.tags && newsItem.tags.length > 0 && (
          <div className="mt-8">
            {" "}
            {/* mt-8 tạo khoảng cách với nội dung chính */}
            <h3 className="text-lg font-semibold mb-3 text-gray-800 dark:text-gray-100">
              Tags:
            </h3>{" "}
            {/* Điều chỉnh kích thước tiêu đề tags */}
        ...
        <div className="mt-12">
          {" "}
          {/* mt-12 tạo khoảng cách lớn với phần trên */}
          <Link href="/tin-tuc">
            <Button
              variant="outline"
      ...
      {/* Kết thúc wrapper giới hạn chiều rộng */}
    </article>
  );
}
